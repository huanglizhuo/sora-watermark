    ...
    <div className="bg-white rounded-lg shadow-sm p-6">
      {status === 'loading' && (
        <div className="flex items-center space-x-3">
          ...
          <span className="text-sm font-medium text-gray-700">Loading FFmpeg...</span>
        </div>
      )}
      ...
      {status === 'processing' && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-gray-500">{Math.round(progress.ratio * 100)}%</span>
          </div>
          ...
          {progress.time > 0 && (
            <p className="text-xs text-gray-500">Processing time: {progress.time.toFixed(1)}s</p>
          )}
        </div>
      )}
